...
#if ADRV9009_DRIVER
            // The ADRV9009_DRIVER Driver must be instantiated last. In this way, when using the FPGA, and when using the GNSS receiver
            // in post-processing mode, the receiver is configured and ready when the DMA starts sending samples to the receiver.
            else if (implementation == "Adrv9009_Fpga_Signal_Source")
                {
                    std::unique_ptr<GNSSBlockInterface> block_ = std::make_unique<Adrv9009FpgaSignalSource>(configuration, role, in_streams,
                        out_streams, queue);
                    block = std::move(block_);
                }
#endif
