/*!
 * \file adrv9009_fpga_signal_source.cc
 * \brief signal source for Analog Devices front-end ADRV9009 connected directly
 * to FPGA accelerators.
 * This source implements only the ADRV9009 control. It is NOT compatible with
...
#include "adrv9009_fpga_signal_source.h"
#include "GPS_L1_CA.h"
#include "GPS_L5.h"
...
Adrv9009FpgaSignalSource::Adrv9009FpgaSignalSource(const ConfigurationInterface *configuration,
    const std::string &role, unsigned int in_stream, unsigned int out_stream,
    Concurrent_Queue<pmt::pmt_t> *queue __attribute__((unused)))
    : SignalSourceBase(configuration, role, "Adrv9009_Fpga_Signal_Source"s),
      queue_(queue),
      ...
      profile_file_(configuration->property(role + ".profile_path", default_profile_path)),
      ...
      freq2_(configuration->property(role + ".freq2", static_cast<uint64_t>(GPS_L5_FREQ_HZ))),
      ...
      enable_DMA_(false),
      enable_dynamic_bit_selection_(configuration->property(role + ".enable_dynamic_bit_selection", false)),
      ...
      repeat_(configuration->property(role + ".repeat", false))
{
    ...
    if (switch_position_ != 0 && switch_position_ != 2)
        {
            std::cout << "SignalSource.switch_position configuration parameter must be either 0: read from file(s) via DMA, or 2: read from ADRV9009\n";
    ...
    if (switch_position_ == 2)  // Real-time via ADRV9009
        {
            ...
            std::cout << "LO frequency for Port A : " << freq_ << " Hz\n";
            std::cout << "LO frequency for Port B : " << freq2_ << " Hz\n";
            try
                {
                    config_adrv9009_rx_local(
			freq_,
			freq2_,
                        rx1_enable_,
                        rx2_enable_,
...
Adrv9009FpgaSignalSource::~Adrv9009FpgaSignalSource()
{
    /* cleanup and exit */
    ...
    if (switch_position_ == 2)  // Real-time via AD9361
        {
            if (rf_shutdown_)
                {
                    std::cout << "* ADRV9009 Disabling RX streaming channels\n";
                    if (!disable_adrv9009_rx_local())
                        {
...
void Adrv9009FpgaSignalSource::start()
{
...
void Adrv9009FpgaSignalSource::run_DMA_process(const std::string &filename0, const std::string &filename1, uint64_t &samples_to_skip, size_t &item_size, int64_t &samples, bool &repeat, uint32_t &dma_buff_offset_pos, Concurrent_Queue<pmt::pmt_t> *queue)
{
...
void Adrv9009FpgaSignalSource::connect(gr::top_block_sptr top_block)
{
    if (top_block)
        { /* top_block is not null */
        };
    DLOG(INFO) << "ADRV9009 FPGA source nothing to connect";
}


void Adrv9009FpgaSignalSource::disconnect(gr::top_block_sptr top_block)
{
    if (top_block)
        { /* top_block is not null */
        };
    DLOG(INFO) << "ADRV9009 FPGA source nothing to disconnect";
}


gr::basic_block_sptr Adrv9009FpgaSignalSource::get_left_block()
{
...
gr::basic_block_sptr Adrv9009FpgaSignalSource::get_right_block()
{
    return {};
}
