    ...
    while ((!current_synchro_data.Flag_valid_symbol_output) && (!d_stop_tracking))
        {
			...
			//std::cout << "Counter Value " << counter_value << '\n';
			//std::cout << "ACQ Delay " << static_cast<uint64_t>((d_acq_sample_stamp + d_acq_code_phase_samples)) << '\n';
			...
			//std::cout << "Absolute samples offset in HW " << absolute_samples_offset << '\n';
