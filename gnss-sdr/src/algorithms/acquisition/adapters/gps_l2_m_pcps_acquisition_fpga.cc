                                ...
                                out_streams_(out_streams)
{
    ...
    fs_in_ = fs_in_ / acq_parameters.downsampling_factor;
    ...
    std::cout << "Code length " << code_length << '\n';
    unsigned int nsamples_total = pow(2, nbits);
    std::cout << "N samples Total " << nsamples_total << '\n';
    unsigned int select_queue_Fpga = configuration->property(role + ".select_queue_Fpga", 1);
    ...
    std::cout << "GPS L2 M Exclude Limit " << acq_parameters.excludelimit << '\n';
