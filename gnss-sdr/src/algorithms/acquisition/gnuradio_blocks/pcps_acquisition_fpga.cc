...
void pcps_acquisition_fpga::acquisition_core(uint32_t num_doppler_bins, uint32_t doppler_step, int32_t doppler_min)
{
    ...
    std::cout << "First Peak " << firstpeak << " \n";
    std::cout << "Second Peak " << secondpeak << " \n";
    std::cout << "Index " << indext << "\n";
    std::cout << "Initial Sample " << initial_sample << " \n";
	*/
    if (total_block_exp > d_total_block_exp)
	    ...
	    //std::cout << "BLOCK Attenuation too HIGH \n";
    ...
    if (d_select_queue_Fpga == 0)
        {
            if (d_downsampling_factor > 1)
                {
                    d_gnss_synchro->Acq_delay_samples = static_cast<double>(d_downsampling_factor * (indext - (d_fft_size - d_acq_parameters.code_length)));
		    if (d_sample_counter == 0)
		        {
                    	    //d_gnss_synchro->Acq_samplestamp_samples = d_downsampling_factor * static_cast<uint64_t>(d_sample_counter);  // 33;  // 41;  // + 81*0.5;  // delay due to the downsampling filter in the acquisition
                    	    d_gnss_synchro->Acq_samplestamp_samples = 0;  // 33;  // 41;  // + 81*0.5;  // delay due to the downsampling filter in the acquisition
			}
		    else
		        {
                    	    d_gnss_synchro->Acq_samplestamp_samples = d_downsampling_factor * static_cast<uint64_t>(d_sample_counter) - static_cast<uint64_t>(64);  // 33;  // 41;  // + 81*0.5;  // delay due to the downsampling filter in the acquisition
                	}
		}
            else
                {
        ...
        {
            d_gnss_synchro->Acq_delay_samples = static_cast<double>(indext-(d_fft_size - d_acq_parameters.code_length));
