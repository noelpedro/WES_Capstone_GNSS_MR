...
signal1 = decimate(signal1, settings.downsampling_factor, 128, 'fir');
signal2 = decimate(signal2, settings.downsampling_factor, 128, 'fir');
% Find sampling period
ts = 1 / (settings.samplingFreq /settings.downsampling_factor);
samplesPerCode = round((settings.samplingFreq/ settings.downsampling_factor) / ...
                        (settings.codeFreqBasis / settings.codeLength));
...
%% Correlate signals ======================================================   
    ...
    caCodeFreqDom = conj(fft(caCodesTable(PRN, :), settings.nfft));
...
%         
        ...
        IQfreqDom1 = fft(I1 + 1j*Q1, settings.nfft);
        %IQfreqDom2 = fft(I2 + 1j*Q2, settings.nfft);
        ...
        acqRes1 = abs(ifft(convCodeIQ1, settings.nfft)) .^ 2;
        %acqRes2 = abs(ifft(convCodeIQ2, settings.nfft)) .^ 2;
        
        %--- Check which msec had the greater power and save that, will
        %"blend" 1st and 2nd msec but will correct data bit issues
        %if (max(acqRes1) > max(acqRes2))
        results(frqBinIndex, :) = acqRes1(settings.nfft - samplesPerCode + 1: end);
        %else
...
%% Look for correlation peaks in the results ==============================
    ...
    samplesPerCodeChip   = round((settings.samplingFreq/settings.downsampling_factor) / settings.codeFreqBasis) + 1;
...
%% Fine resolution frequency search =======================================
        ...
        disp(frequencyBinIndex);
        disp(codePhase);
        acqResults.codePhase(PRN) = codePhase * settings.downsampling_factor;
        acqResults.carrFreq(PRN)  = (-settings.acqSearchBand * 1000/2) + settings.acqSearchBin*(frequencyBinIndex-1);
